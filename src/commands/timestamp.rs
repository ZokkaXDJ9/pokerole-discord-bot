...
pub async fn timestamp(
    ...
    match user {
        Ok(user) => {
            if user.setting_time_offset_hours.is_none() || user.setting_time_offset_minutes.is_none() {
                send_settings_hint(&ctx).await?;
            }
            print_timestamp(&ctx, minute, hour, day, month, year,
                            user.setting_time_offset_hours.unwrap_or_default(),
                            user.setting_time_offset_minutes.unwrap_or_default()).await
        }
        Err(_) => {
            send_settings_hint(&ctx).await?;
...
async fn print_timestamp<'a>(ctx: &Context<'a>, minute: Option<u8>, hour: Option<u8>, day: Option<u8>, month: Option<u8>, year: Option<u16>,
                             hour_offset: i64, minute_offset: i64)
    -> Result<(), Error> {
