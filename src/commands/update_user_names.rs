...
pub async fn update_user_names(ctx: Context<'_>) -> Result<(), Error> {
    send_ephemeral_reply(
        &ctx,
        "## Updating user data\nStep 1/2: Fetching all user names from discord...",
    )
    .await?;
    ...
    if let Ok(records) = records {
        for record in records {
            let user = UserId::from(record.user_id as u64).to_user(ctx).await;
            if let Ok(user) = user {
                // TODO: Once serenity supports the new display names, default to those instead.
                let nickname = user
                    .nick_in(ctx, GuildId::from(record.guild_id as u64))
                    .await
                    .unwrap_or(user.name.clone());
                let result = sqlx::query!(
                    "UPDATE user_in_guild SET name = ? WHERE user_id = ? AND guild_id = ?",
                    nickname,
                    record.user_id,
                    record.guild_id,
                )
                ...
                if result.is_err() {
                    send_ephemeral_reply(
                        &ctx,
                        &format!(
                            "Failed to set up user_in_guild row for <@{}> ???",
                            record.user_id
                        ),
                    )
                    .await?;
                ...
                send_ephemeral_reply(
                    &ctx,
                    &format!("Unable to get user for <@{}>", record.user_id),
                )
                .await?;
            }
        }
